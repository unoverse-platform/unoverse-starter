# @unoverse-platform/email

## 1.0.9

### Patch Changes

- SendEmail: `whenToUse` now promotes HTML email bodies (set `bodyType='html'` and have the upstream LLM produce real HTML — headings, links — not a wall of plain text) and clarifies the email body is its OWN dedicated one-shot LLM node (one node, one task) — never reused from a node doing another job (e.g. a document author).
