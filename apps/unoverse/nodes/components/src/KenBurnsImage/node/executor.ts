...
export default class KenBurnsImageExecutor extends PromiseNode {
  ...
  protected async executeNode(
    ...
    const firstRender = !KenBurnsImageExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (KenBurnsImageExecutor.loggedRenders.size > 10000) {
        KenBurnsImageExecutor.loggedRenders.clear();
        KenBurnsImageExecutor.lastPublished.clear();
      }
      KenBurnsImageExecutor.loggedRenders.add(renderKey);
    ...
    const prev = KenBurnsImageExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    KenBurnsImageExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
