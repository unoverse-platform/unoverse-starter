...
export default class CardExecutor extends PromiseNode {
  ...
  protected async executeNode(
    ...
    const firstRender = !CardExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (CardExecutor.loggedRenders.size > 10000) {
        CardExecutor.loggedRenders.clear();
        CardExecutor.lastPublished.clear();
      }
      CardExecutor.loggedRenders.add(renderKey);
    ...
    const prev = CardExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    CardExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
