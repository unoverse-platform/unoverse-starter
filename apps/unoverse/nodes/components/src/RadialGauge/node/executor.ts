...
import { RadialGaugeConfig, RadialGaugeOutput } from "../util/types";
import { loadDefaultTemplate } from "../service/templates";
import { publishComponent } from "../service/publishComponent";

export default class RadialGaugeExecutor extends PromiseNode {
  ...
  protected async validateConfig(config: RadialGaugeConfig): Promise<ValidationResult> {
    return { success: true };
  }
  ...
  protected async executeNode(
  ...
  ): Promise<RadialGaugeOutput> {
    // One log + one first-render marker per (execution, node) pair.
    const renderKey = `${context.executionId}:${context.nodeId}`;
    const firstRender = !RadialGaugeExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (RadialGaugeExecutor.loggedRenders.size > 10000) {
        RadialGaugeExecutor.loggedRenders.clear();
        RadialGaugeExecutor.lastPublished.clear();
      }
      RadialGaugeExecutor.loggedRenders.add(renderKey);
      this.logger.info(`✅ [RadialGauge] ComponentSpec generated for node: ${context.nodeId}`);
    }
    ...
    if (config.pct !== undefined) {
      props.pct = config.pct;
    } else if (firstRender) {
      props.pct = "";
    }
    ...
    const prev = RadialGaugeExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    RadialGaugeExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
