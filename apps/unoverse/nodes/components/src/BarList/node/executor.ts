...
import { BarListConfig, BarListOutput } from "../util/types";
import { loadDefaultTemplate } from "../service/templates";
import { publishComponent } from "../service/publishComponent";

export default class BarListExecutor extends PromiseNode {
  ...
  protected async validateConfig(config: BarListConfig): Promise<ValidationResult> {
    return { success: true };
  }
  ...
  protected async executeNode(
  ...
  ): Promise<BarListOutput> {
    // One log + one first-render marker per (execution, node) pair.
    const renderKey = `${context.executionId}:${context.nodeId}`;
    const firstRender = !BarListExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (BarListExecutor.loggedRenders.size > 10000) {
        BarListExecutor.loggedRenders.clear();
        BarListExecutor.lastPublished.clear();
      }
      BarListExecutor.loggedRenders.add(renderKey);
      this.logger.info(`✅ [BarList] ComponentSpec generated for node: ${context.nodeId}`);
    }
    ...
    const prev = BarListExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    BarListExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
