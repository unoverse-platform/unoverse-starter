...
export default class BarChartExecutor extends PromiseNode {
  ...
  protected async executeNode(
    ...
    const firstRender = !BarChartExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (BarChartExecutor.loggedRenders.size > 10000) {
        BarChartExecutor.loggedRenders.clear();
        BarChartExecutor.lastPublished.clear();
      }
      BarChartExecutor.loggedRenders.add(renderKey);
    ...
    const prev = BarChartExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    BarChartExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
