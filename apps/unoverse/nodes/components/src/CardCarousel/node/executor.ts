...
export default class CardCarouselExecutor extends PromiseNode {
  ...
  protected async executeNode(
    ...
    const firstRender = !CardCarouselExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (CardCarouselExecutor.loggedRenders.size > 10000) {
        CardCarouselExecutor.loggedRenders.clear();
        CardCarouselExecutor.lastPublished.clear();
      }
      CardCarouselExecutor.loggedRenders.add(renderKey);
    ...
    const prev = CardCarouselExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    CardCarouselExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
