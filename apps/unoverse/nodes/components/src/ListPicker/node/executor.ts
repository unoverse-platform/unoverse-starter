...
export default class ListPickerExecutor extends PromiseNode {
  ...
  protected async executeNode(
    ...
    const firstRender = !ListPickerExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (ListPickerExecutor.loggedRenders.size > 10000) {
        ListPickerExecutor.loggedRenders.clear();
        ListPickerExecutor.lastPublished.clear();
      }
      ListPickerExecutor.loggedRenders.add(renderKey);
    ...
    if (config.elements !== undefined) {
      props.elements = config.elements;
    }
    ...
    const prev = ListPickerExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    ListPickerExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
