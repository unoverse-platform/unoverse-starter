...
export default class MarkdownRendererExecutor extends PromiseNode {
  ...
  protected async executeNode(
    ...
    const firstRender = !MarkdownRendererExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (MarkdownRendererExecutor.loggedRenders.size > 10000) {
        MarkdownRendererExecutor.loggedRenders.clear();
        MarkdownRendererExecutor.lastPublished.clear();
      }
      MarkdownRendererExecutor.loggedRenders.add(renderKey);
    ...
    if (config.streamingLabel !== undefined) {
      props.streamingLabel = config.streamingLabel;
    } else if (firstRender) {
      props.streamingLabel = "";
    }
    if (config.placeholderText !== undefined) {
      props.placeholderText = config.placeholderText;
    } else if (firstRender) {
    ...
    const prev = MarkdownRendererExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    MarkdownRendererExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
