...
import { StackedBarChartConfig, StackedBarChartOutput } from "../util/types";
import { loadDefaultTemplate } from "../service/templates";
import { publishComponent } from "../service/publishComponent";

export default class StackedBarChartExecutor extends PromiseNode {
  ...
  protected async validateConfig(config: StackedBarChartConfig): Promise<ValidationResult> {
    return { success: true };
  }
  ...
  protected async executeNode(
    ...
    const firstRender = !StackedBarChartExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (StackedBarChartExecutor.loggedRenders.size > 10000) {
        StackedBarChartExecutor.loggedRenders.clear();
        StackedBarChartExecutor.lastPublished.clear();
      }
      StackedBarChartExecutor.loggedRenders.add(renderKey);
      this.logger.info(`✅ [StackedBarChart] ComponentSpec generated for node: ${context.nodeId}`);
    }
    ...
    const prev = StackedBarChartExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    StackedBarChartExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
