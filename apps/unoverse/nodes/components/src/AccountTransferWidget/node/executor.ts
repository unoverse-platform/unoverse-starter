...
export default class AccountTransferWidgetExecutor extends PromiseNode {
  ...
  protected async executeNode(
    ...
    const firstRender = !AccountTransferWidgetExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (AccountTransferWidgetExecutor.loggedRenders.size > 10000) {
        AccountTransferWidgetExecutor.loggedRenders.clear();
        AccountTransferWidgetExecutor.lastPublished.clear();
      }
      AccountTransferWidgetExecutor.loggedRenders.add(renderKey);
    ...
    const prev = AccountTransferWidgetExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    AccountTransferWidgetExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
