...
export default class CardFinderExecutor extends PromiseNode {
  ...
  protected async executeNode(
    ...
    const firstRender = !CardFinderExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (CardFinderExecutor.loggedRenders.size > 10000) {
        CardFinderExecutor.loggedRenders.clear();
        CardFinderExecutor.lastPublished.clear();
      }
      CardFinderExecutor.loggedRenders.add(renderKey);
    ...
    if (config.recommendation !== undefined) {
      props.recommendation = config.recommendation;
    }
    if (config.eligibleTiers !== undefined) {
      props.eligibleTiers = config.eligibleTiers;
    }
    ...
    const prev = CardFinderExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    CardFinderExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
