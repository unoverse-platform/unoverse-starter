...
export default class MetricGridExecutor extends PromiseNode {
  ...
  protected async executeNode(
    ...
    const firstRender = !MetricGridExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (MetricGridExecutor.loggedRenders.size > 10000) {
        MetricGridExecutor.loggedRenders.clear();
        MetricGridExecutor.lastPublished.clear();
      }
      MetricGridExecutor.loggedRenders.add(renderKey);
    ...
    const prev = MetricGridExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    MetricGridExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
