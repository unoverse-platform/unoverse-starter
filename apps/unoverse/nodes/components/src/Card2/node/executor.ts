...
export default class Card2Executor extends PromiseNode {
  ...
  protected async executeNode(
    ...
    const firstRender = !Card2Executor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (Card2Executor.loggedRenders.size > 10000) {
        Card2Executor.loggedRenders.clear();
        Card2Executor.lastPublished.clear();
      }
      Card2Executor.loggedRenders.add(renderKey);
    ...
    const prev = Card2Executor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    Card2Executor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
