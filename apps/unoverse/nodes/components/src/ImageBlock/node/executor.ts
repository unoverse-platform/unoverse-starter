...
export default class ImageBlockExecutor extends PromiseNode {
  ...
  protected async executeNode(
    ...
    const firstRender = !ImageBlockExecutor.loggedRenders.has(renderKey);
    if (firstRender) {
      if (ImageBlockExecutor.loggedRenders.size > 10000) {
        ImageBlockExecutor.loggedRenders.clear();
        ImageBlockExecutor.lastPublished.clear();
      }
      ImageBlockExecutor.loggedRenders.add(renderKey);
    ...
    const prev = ImageBlockExecutor.lastPublished.get(renderKey);
    const changed: Record<string, any> = {};
    for (const [k, v] of Object.entries(props)) {
      if (!prev || JSON.stringify(prev[k]) !== JSON.stringify(v)) changed[k] = v;
    }
    ImageBlockExecutor.lastPublished.set(renderKey, { ...(prev ?? {}), ...props });
