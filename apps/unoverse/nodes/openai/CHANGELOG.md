...
- OpenAIAgent: fix stream event shapes — the Agents SDK normalizes text deltas to 'output_text_delta' and wraps raw Responses events as {type:'model', event}. Text streaming (chunk) now actually streams, and reasoning summary deltas are read from the raw pass-through.

## 1.1.13

### Patch Changes

- OpenAIAgent: emit reasoning — request reasoning summaries (new reasoningSummary config, default auto) and stream response.reasoning_summary_text.delta to the reasoning output; final reasoning included in outputs.

## 1.1.12

### Patch Changes

- OpenAIAgent: wire reasoningEffort + verbosity through the Agents SDK (modelSettings.reasoning.effort / text.verbosity), with xhigh in the effort enum. Previously these were silently ignored on the SDK-based agent.
