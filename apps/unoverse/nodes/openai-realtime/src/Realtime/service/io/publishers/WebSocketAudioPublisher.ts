...
export class WebSocketAudioPublisher {
  ...
  async publishAudio(config: {
    ...
    if (config.audioState === "SPEECH_STARTED" || config.audioState === "SPEECH_ENDED") {
      // Flush any buffered audio BEFORE the end signal so the final frames land on the
      // wire ahead of SPEECH_ENDED (else the client unmutes / stops the avatar while a
      // trailing chunk is still in flight). Matches publishState's ordering.
