...
function getLogger() {
  return getPlatformDependencies().createLogger("GrokWebSocketAudioPublisher");
}
...
export class WebSocketAudioPublisher {
  ...
  async publishAudio(config: {
    ...
    this.addToBuffer(config.conversationId, audioBuffer, config.metadata);
  }

  private addToBuffer(conversationId: string, chunk: Buffer, _metadata: StreamingMetadata): void {
  ...
  async publishState(config: {
    ...
    // Merge additionalMetadata (toolName, callId, etc.) into metadata so the
    // client receives it via event.metadata in onAudioState callbacks.
